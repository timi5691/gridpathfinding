...
pub fn (grid Grid) is_pos_in_grid(pos Vec2) bool {
	return grid.is_gridpos_in_grid(grid.pixelpos_to_gridpos(pos))
}

pub fn (grid Grid) is_gridpos_in_grid(grpos Vec2) bool {
	return grpos.x >= 0 && grpos.x <= grid.cols && grpos.y >= 0 && grpos.y <= grid.rows
}
...
pub fn (grid Grid) get_cells_around_in_steps(cell int, steps int) []int {
	mut rs := [cell]
	gridpos := grid.id_to_gridpos(cell)
	for c in -steps .. steps {
		for r in -steps .. steps {
			gp := Vec2{c + gridpos.x, r + gridpos.y}
			id := grid.gridpos_to_id(gp)
			if grid.is_gridpos_in_grid(gp) {
				dist := grid.calc_steps(cell, id)
				if dist <= steps {
					rs << id
				}
			}
...
pub fn (grid Grid) get_cells_around_in_steps_except_visible_cells(cell int, steps int, visible_cells_map map[int]bool) []int {
	mut rs := [cell]
	gridpos := grid.id_to_gridpos(cell)
	for c in -steps .. steps {
		for r in -steps .. steps {
...
pub fn (mut grid Grid) draw_visible_cell(ctx gg.Context, visible_cells_map map[int]bool) {
	for cell_id, _ in visible_cells_map {
		if cell_id !in grid.not_walkable_ids {
			cell_pos := grid.id_to_pixelpos(cell_id, false).minus(grid.cam_pos)
			ctx.draw_rect_filled(cell_pos.x, cell_pos.y, grid.cell_size, grid.cell_size,
				gx.gray)
			continue
		}
		cell_pos := grid.id_to_pixelpos(cell_id, false).minus(grid.cam_pos)
		ctx.draw_rect_filled(cell_pos.x, cell_pos.y, grid.cell_size, grid.cell_size, gx.purple)
	}
}
