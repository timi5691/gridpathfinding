...
pub fn load_assets(mut game Game) {
	...
	cursors_img := game.ctx.create_image(asset.get_path('assets/imgs', 'cursors.png')) or { panic(err) }
	game.img_map = {
		'cursors':   &cursors_img
		'items':     &items_img
...
pub fn process(mut game Game) {
	...
	for _, mut gobj in game.gobj_map {
		...
		if atk_eid != -1 {
			...
			if gobj.in_cam_view {
				if mut sp := game.sound_player_map['laserShoot'] {
					spawn sp.play('assets/audio', 'laserShoot.wav')
				}
			}
		...
		if gobj.is_hp_changed() && gobj.in_cam_view {
			if gobj.is_hp_zero() {
				if mut sp := game.sound_player_map['explosion'] {
					spawn sp.play('assets/audio', 'explosion.wav')
				}
			} else {
				if mut sp := game.sound_player_map['laserShoot'] {
					spawn sp.play('assets/audio', 'hitHurt.wav')
				}
			}
		}
		...
		if gobj.team == 1 && gobj.hp > 0 && gobj.in_cam_view {
			steps_can_see := 5
			can_see_cells := game.grid.get_cells_around_in_steps_except_visible_cells(gobj.cur_cell, steps_can_see, game.visible_cells_map)
