...
import gg
import gx
import my_extra { Rect, Vec2, is_rect_in_rect }
...
pub fn (select_area SelectArea) draw_self(mut ctx gg.Context) {
	if !select_area.visible {
		return
	}
	ctx.draw_rect_empty(int(select_area.renx), int(select_area.reny), int(select_area.renw),
		int(select_area.renh), gx.green)
}
