...
pub fn (mut itlist ItemList) remove_item(it_name string) {
	if !itlist.is_item_exists(it_name) {
		return
	}
...
pub fn (mut itlist ItemList) draw_items(mut ctx gg.Context) {
	...
	if itlist.selected_slot != -1 {
		...
		ctx.draw_rect_empty(itlist.gui_x + 1, cursor_y + 1, itlist.width - 2, slot_height - 2,
			gx.rgba(255, 255, 255, 200))
	}
