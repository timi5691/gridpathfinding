...
import os
import os.asset
import my_extra { Vec2, is_pos_in_rect, is_rect_in_rect, rad_to_deg, deg_to_rad }
...
import md_gobj { Gobj }
import md_explosion {Explosion, Particle, new_explosion}
...
pub fn process(mut game Game) {
	...
	game.cam.update(Vec2{game.grid.cols * game.grid.cell_size, game.grid.rows * game.grid.cell_size}, game.mouse_gui_pos, game.delta)
	...
	for _, mut gobj in game.gobj_map {
		...
		uex.check_gobj_player_selected(mut gobj, game.left_mouse_pressed, game.left_mouse_released, game.select_area.rect,
			game.mouse_pos, game.grid.cell_size, game.double_click_selecting, game.double_click_select_gteam, game.double_click_select_gtype)
		//
		gobj.check_is_hurt()
...
pub fn draw(mut game Game) {
	...
	for mut gobj in in_cam_view_gobj_list {
		if is_pos_in_rect(gobj.cur_pos, game.cam.rect) {
			...
			if gobj.hp > 0 {
				...
				cl.a = 255
				for i in 0..hp_cell {
					ctx.draw_rect_empty(draw_pos.x  - game.grid.cell_size/2.0 + i*a, draw_pos.y - game.grid.cell_size/2.0 - 8, a, 8, gx.white)	
				}
				ctx.draw_rect_filled(draw_pos.x  - game.grid.cell_size/2.0, draw_pos.y - game.grid.cell_size/2.0 - 8, gobj.hp/gobj.max_hp*game.grid.cell_size, 8, cl)
	...
	mut fog_cl := gx.black
	fog_cl.a = 70
	ctx.draw_rect_filled(1, 1, ws.width, ws.height, fog_cl)
