...
import gg
import gx
import my_extra {Vec2, Rect, guipos_to_realpos, is_pos_in_rect, rad_to_deg, deg_to_rad}
import md_grid {Grid}
...
pub fn (mut gobj Gobj) control_aspr_rot(gobj_map map[int]Gobj) {
	if gobj.state in ['attacking nearest enemy'] {
...
pub fn (mut gobj Gobj) check_is_hurt() {
			...
			if gobj.nearest_eid != -1 {
				if e := gobj_map[gobj.nearest_eid] {
					...
					if e_dist <= gobj.attack_range*gobj.attack_range {
						...
						gobj.set_new_dest_pos(e.dest_pos)
